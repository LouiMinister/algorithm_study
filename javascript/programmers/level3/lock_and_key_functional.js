...
function solution(key, lock){
    const pad = (size) => Array(size).fill(null);
    const addPadding = (key, size) => {
        const len = key.length + size;
        return Array(len).fill(null)
            .map( (_, row) => row < key.length ? 
                [...key[row], ...pad(size)] : 
                [...pad(len)]);
    }
    const rotate = (key) => {
    ...
    //console.log(addPadding(key,2));
    console.log(rotate(addPadding(key,2)));
}