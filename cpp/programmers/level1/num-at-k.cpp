...
입출력 예 설명
[1, 5, 2, 6, 3, 7, 4]를 2번째부터 5번째까지 자른 후 정렬합니다. [2, 3, 5, 6]의
세 번째 숫자는 5입니다. [1, 5, 2, 6, 3, 7, 4]를 4번째부터 4번째까지 자른 후
정렬합니다. [6]의 첫 번째 숫자는 6입니다. [1, 5, 2, 6, 3, 7, 4]를 1번째부터
7번째까지 자릅니다. [1, 2, 3, 4, 5, 6, 7]의 세 번째 숫자는 3입니다.
*/
...
vector<int> solution(vector<int> array, vector<vector<int>> commands) {
  ...
  for (int i = 0; i < commands.size(); i++) {
    vector<int> splicedVector(array.begin() + commands[i][0] - 1,
                              array.begin() + commands[i][1]);
    sort(splicedVector.begin(), splicedVector.end());
    answer.push_back(splicedVector[commands[i][2] - 1]);
  }
  return answer;
}
...
int main() {
  ...
  vector<int> answer = solution2(a, b);
